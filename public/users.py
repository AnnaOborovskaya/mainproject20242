...
@users_router.post("/api/courier", response_model=Union[Main_User_2], status_code=status.HTTP_201_CREATED)
...
@users_router.get("/api/courier/{id}", response_model=Union[Main_User_5, Main_Order_2])
...
async def edit_courier(item: Annotated[Main_User_2, Body(embed=True, description="Изменяем данные курьера по id")], DB: AsyncSession = Depends(get_session)):
    '''Изменение данных о курьере'''
